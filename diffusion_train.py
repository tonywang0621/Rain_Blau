...
class Trainer():
    ...
    def _valid(self, sharp, blur, flow):
        ...
        output_01 = (output.detach() + 0.5).clamp(0, 1)#
        blur_01 = (blur.detach() + 0.5).clamp(0, 1) #

        psnr = torch.mean(self.psnr_func(output_01.detach(), blur_01.detach())).item()# 改成output01 blur01
        lpips = torch.mean(self.lpips_func(output_01.detach(), blur_01.detach())).item()#
        return psnr, lpips
    ...
    def val_save_image(self, dir_path, dataset, val_num=3):
        ...
        val_num = min(val_num, len(dataset))#

        if val_num == 0:
            print("Validation dataset is empty, skip saving images.")#
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path",default="./dataset/RAIN_data",type=str)
    parser.add_argument("--flow_data_path",default="./dataset/RAIN_flow",type=str)
    parser.add_argument("--dir_path", default="./experiments/ID_Blau", type=str)
    parser.add_argument("--flow_norm",default=False,type=bool)
